...
Manipulation is easiest to understand by driving it. The humanoid below runs the
full **reach → grasp → move → release** loop described in this chapter, with the
cube as a real physics object. Ask it to walk over, pick the cube up, carry it,
and then set it down or throw it.

<RobotLab />

:::tip Commands to try
`pick up the cube` · `pick up the cube and throw it` · `kick the cube` ·
`walk to the cube then put it down`. Watch the approach, grasp, and release
phases separate out just like the pick-and-place pipeline above.
:::

## Review Flashcards

Flip through the deck to lock in the key ideas. Mark each card **Got it** or **Review** — the deck remembers your progress on this device.

<FlashcardDeck
  title="Chapter 12 — Manipulation"
  deckId="ch12-manipulation"
  cards={  [
      {
          "front": "What is a grasp?",
          "back": "A set of contacts on an object that together hold it against gravity and disturbance."
      },
      {
          "front": "Force closure vs. form closure?",
          "back": "Force closure resists arbitrary forces through friction at the contacts; form closure holds the object purely by geometry, no friction needed."
      },
      {
          "front": "What is the friction cone?",
          "back": "The set of forces a contact can transmit — tangential force limited by μ·Fₙ."
      },
      {
          "front": "What is an antipodal grasp?",
          "back": "Two opposite contacts whose normals point at each other along the same line."
      },
      {
          "front": "What is a pre-grasp pose?",
          "back": "The approach configuration the hand takes just before closing on the object."
      },
      {
          "front": "What is whole-body manipulation?",
          "back": "Using legs, torso, and arms together to push, carry, or stabilize large objects."
      }
  ]}
/>
