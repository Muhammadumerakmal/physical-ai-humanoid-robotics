...
import Exercise from '@site/src/components/Educational/Exercise';
import FlashcardDeck from '@site/src/components/Educational/Flashcard';
...
<Quiz 
  ...
  options={[
    ...
    "Physical AI operates on symbols; digital AI does not.",
    "Physical AI is embodied and subject to physical constraints like gravity and friction.",
...
<Exercise title="Mapping the Loop">
  Take a simple robot (like a vacuum cleaner). List the components that act as sensors, the decision-making logic, and the actuators. Sketch out the perception-action loop for this specific robot.
</Exercise>
...
that closes the <Term>perception–action loop</Term> is the moat.
...
The loop made concrete: a physical AI system is a pipeline from sensors to
actuators, with an AI model at its heart — and the world closing the loop back
to the sensors.
...
<Quiz
  question="What most clearly separates a physical AI system from a digital AI system?"
  options={[
    'It runs on a faster computer',
    'It closes a perception-action loop in a physical body',
...
Flip through the deck to lock in the key ideas. Mark each card **Got it** or
**Review** — the deck remembers your progress on this device.

<FlashcardDeck
  title="Chapter 1 — Core Ideas"
  deckId="ch1-what-is-physical-ai"
  cards={[
    {
      front: 'What is physical AI?',
      back: 'AI that senses, decides, and acts in the physical world through a body — subject to gravity, friction, contact, and energy.',
    },
    {
      front: 'How does physical AI differ from digital AI?',
      back: 'Digital AI manipulates symbols/pixels/tokens with cheap, instant retries. Physical AI acts on atoms in real time, where mistakes have physical consequences.',
    },
    {
      front: 'What are the three abilities bundled in physical AI?',
      back: 'Perception (read the world via sensors), cognition (make sense of it and choose), and action (commit to the world via actuators) — all in one body, in real time.',
    },
    {
      front: 'What is the perception–action loop?',
      back: 'The continuous cycle of sense → decide → act → sense again that every embodied system must close in real time.',
    },
    {
      front: 'Why do humanoids sit at the far end of the physical-AI spectrum?',
      back: 'They trade peak performance on any single task for generality — a body that can attempt the widest range of human tasks and environments.',
    },
    {
      front: 'What is sim-to-real?',
      back: 'Training a model in simulation and transferring the learned behavior onto real hardware.',
    },
  ]}
/>
