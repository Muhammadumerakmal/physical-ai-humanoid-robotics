...
/** Render a LaTeX string to KaTeX HTML, failing soft to the raw source. */
...
/** Block (display) math element. */
function MathBlock({tex}: {tex: string}) {
  return (
    <div
      className={styles.mathBlock}
      // KaTeX output is generated by the trusted katex library, not user HTML.
      dangerouslySetInnerHTML={{__html: katexHtml(tex.trim(), true)}}
    />
  );
}
...
/** Split raw assistant text into inline code / bold / math / link / text spans. */
function inlineSpans(text: string): InlineSpan[] {
  // Order matters: inline code is matched first so `$` inside code isn't math.
  ...
  return inlineSpans(text).map((span, i) => {
    ...
    if (span.type === 'math') {
      return (
        <span
          key={i}
          // KaTeX output is generated by the trusted katex library.
          dangerouslySetInnerHTML={{__html: katexHtml(span.value, false)}}
        />
      );
    }
    if (span.type === 'link') {
...
function MessageBubble({message}: {message: Message}) {
  ...
  parts.forEach((part, i) => {
    if (i % 2 === 1) {
      ...
      // Pull display math ($$...$$) out of the prose into its own blocks.
      part.split(/\$\$([\s\S]*?)\$\$/g).forEach((seg, si) => {
        if (si % 2 === 1) {
          if (seg.trim()) nodes.push(<MathBlock key={`${i}-m${si}`} tex={seg} />);
        } else if (seg.trim()) {
          nodes.push(...proseBlock(seg, `${i}-${si}`));
        }
      });
    }
  });
...
export default function BookAgent() {
  ...
  // Let any page open the assistant with a preset question (e.g. the
  // "Ask the AI about this chapter" button in the chapter banner). A ref keeps
  // the listener pointed at the latest `send` without re-subscribing.
  ...
  useEffect(() => {
    function onAsk(e: Event) {
      ...
      setOpen(true);
      if (q) window.setTimeout(() => void sendRef.current(q), 150);
    }
    window.addEventListener('bookagent:ask', onAsk);
    return () => window.removeEventListener('bookagent:ask', onAsk);
  }, []);
